...
func (s *Service) URLGetNumber() string {
	return fmt.Sprintf("https://smshub.org/stubs/handler_api.php?api_key=%s&action=getNumber&service=%s&country=%s", s.APIKey, s.ID, s.Country)

}

// послать запрос на смс сервер для отмены активации
func (s *Service) URLCancelOrder(id string) string {
	return fmt.Sprintf("https://smshub.org/stubs/handler_api.php?api_key=%s&action=setStatus&status=8&id=%s", s.APIKey, id)
}

func (s *Service) URLGetStatus(id string) string {
	return fmt.Sprintf("https://smshub.org/stubs/handler_api.php?api_key=%s&action=getStatus&id=%s", s.APIKey, id)
}
